...
#include "Enemy/Animation/Notify/AN_SilverDevil_Skl02_01.h"
...
UAN_SilverDevil_Skl02_01::UAN_SilverDevil_Skl02_01()
{
...
void UAN_SilverDevil_Skl02_01::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation, const FAnimNotifyEventReference& EventReference)
{
	...
	FVector Location = SkeletalMeshComponent->GetSocketLocation(SocketName::Root);
	FRotator Rotation = SkeletalMeshComponent->GetSocketRotation(SocketName::Root);
	...
	bool bATKSuccess = UKismetSystemLibrary::SphereTraceSingleByProfile(
		...
		200, ProfileName, false, IgnoreActors, EDrawDebugTrace::ForDuration,
		HitResult, true);
	...
	if (bATKSuccess)
	{
		...
		UGameplayStatics::ApplyDamage(DamagedActor, ATKFinal, Enemy->GetController(), Enemy, USkillDamageType::StaticClass());
	}
